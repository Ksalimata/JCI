<!DOCTYPE html>
<html>
<head>
	<?php $this->load->view('tlp/css_file'); ?>
    <style type="text/css">
        body{
           font-family: "Opificio" !important;
           font-size: 20px;
        }
        .page-nav h2 {
            font-size: 100px;
            width: 100%;
            color: #fff;
            font-weight: bold;
        }
    </style>
</head>
<body>
	<?php $this->load->view('tlp/header'); ?>

	<!--  ************************* Page Title Starts Here ************************** -->
	<div class="page-nav no-margin row" style="background-image: url('<?=base_url(); ?>assets/images/archives.jpg');height: 400px;width: 100%;background-size: cover; background-position: center center;transform: translate3d(0px, 0px, 0px);background-blend-mode: darken;">
        <div class="container">
            <div class="row">
                <h2>Reunion Culture loisirs et Anglais</h2>
                <!-- <ul>
                    <li> <a href="#"><i class="fas fa-home"></i> Accueil</a></li>
                    <li><i class="fas fa-angle-double-right"></i> Archives</li>
                </ul> -->
            </div>
        </div>
    </div>

	<section class="our-blog" style="background-color: #edf1f5;">
            <div class="container">
                <div class="row session-title">
                    <!-- <p>Take a look at what people say about US </p> -->
                </div>
                <div class="blog-row row" style="">
                    <div class="col-md-12 col-sm-12">
                        <div class="single-blog">
                            <div class="blog-detail">
                                    <h4>
                                        <center><a href="<?php echo site_url('CL');?>">Reunion Culture Loisirs</a></center>
                                    </h4>
                            </div>
                            <figure>
                                <table class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Jour</th>
                                            <th>Activités</th>
                                            <th>Lieu</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        
                                            <tr><th colspan="3" style="text-align:center;">NOVEMBRE 2021</th></tr>
                                            <tr>
                                                <td>08 Novembre 2021</td>
                                                <td>Demarrage des Commissions</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>29 Novembre 2021</td>
                                                <td>Première réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">DECEMBRE 2021</th></tr>
                                            <tr>
                                                <td>13 Decembre 2021</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>29 Decembre 2021</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">JANVIER 2022</th></tr>
                                            <tr>
                                                <td>17 Janvier 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                
                                                <td>31 Janvier 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">FEVRIER 2022</th></tr>
                                            <tr>
                                                <td>14 Février 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>28 Février 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">MARS 2022</th></tr>
                                            <tr>
                                                <td>14 Mars 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>28 Mars 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">AVRIL 2022</th></tr>
                                            <tr>
                                                <td>11 Avril 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>25 Avril 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">MAI 2022</th></tr>
                                            <tr>
                                                <td>09 Mai 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>23 Mai 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">JUIN 2022</th></tr>
                                            <tr>
                                                <td>06 Juin 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>20 Juin 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">JUILLET 2022</th></tr>
                                            <tr>
                                                <td>04 juillet 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr>
                                                <td>18 juillet 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            <tr><th colspan="3" style="text-align:center;">Août 2022</th></tr>
                                            <tr>
                                                <td>01 Août 2022</td>
                                                <td>Réunion de commission</td>
                                                <td>Siège Abidjan-Ivoire</td>
                                            </tr>
                                            
                                    </tbody>
                                </table>
                                
                            </figure>
                        </div>
                    </div>
                </div>
            </div>
    </section>    
	<?php $this->load->view('tlp/footer'); ?>
	<?php $this->load->view('tlp/js_file'); ?>
</body>
</html>